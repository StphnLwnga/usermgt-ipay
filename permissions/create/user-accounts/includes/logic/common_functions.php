<?php
 ...
 //Declare functions for Finance role based on loginbyId function declared below 
  function isSupport($user_id) {
    global $conn;
  ...
  function loginById($user_id) {
    ...
    if (!empty($user)) {
     ...
     if (isAdmin($user_id)) {
        ...
        header('location: ' . BASE_URL . 'csupport/index.php');
       
      }
        else if (isSupport($user_id)) {
        $permissionsSql = "SELECT p.name as permission_name FROM permissions as p
                            JOIN permission_role as pr ON p.id=pr.permission_id
                            WHERE pr.role_id=?";
        $userPermissions = getMultipleRecords($permissionsSql, "i", [$user['role_id']]);
        $_SESSION['userPermissions'] = $userPermissions;
        header('location: ' . BASE_URL . 'finance/index.html');
